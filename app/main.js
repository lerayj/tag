...
function chooseEmailCapture(config, websiteId, userId, sessionId){
    ...
    //console.log('currentUrl: ', currentUrl, " config.emails.register.url: ", config.emails.register.url);
...
function saveMail(selector, type, websiteId, userId, sessionId){
    ...
    //console.log("mail select: ", mail, " selector: ", selector);
    ...
    if(mail.length != 0){
        ...
        mail[0].addEventListener("input", function (e) {
            if(timerTest)
                clearInterval(timerTest);
            var mailToCheck = this.value;
            if(email_validator.validate(mailToCheck)){
                timerTest = setTimeout(() => {
                    var update = {value: mailToCheck, saved_at: firebase.database.ServerValue.TIMESTAMP};
